...
public class TrackDao extends AbstractDao<Track, Long> {
    ...
    */
    public static class Properties {
        ...
        public final static Property Block_id = new Property(4, Long.class, "block_id", false, "BLOCK_ID");
        public final static Property File_id = new Property(5, Long.class, "file_id", false, "FILE_ID");
    };

    private DaoSession daoSession;

    private Query<Track> block_TrackListQuery;
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        ...
        db.execSQL("CREATE TABLE " + constraint + "'TRACK' (" + //
                ...
                "'FILE' TEXT," + // 3: file
                "'BLOCK_ID' INTEGER," + // 4: block_id
                "'FILE_ID' INTEGER);"); // 5: file_id
    }
    ...
    protected void bindValues(SQLiteStatement stmt, Track entity) {
        ...
        if (block_id != null) {
            stmt.bindLong(5, block_id);
        }
 
        Long file_id = entity.getFile_id();
        if (file_id != null) {
            stmt.bindLong(6, file_id);
        }
    }
    ...
    public Track readEntity(Cursor cursor, int offset) {
        Track entity = new Track( //
            ...
            cursor.isNull(offset + 4) ? null : cursor.getLong(offset + 4), // block_id
            cursor.isNull(offset + 5) ? null : cursor.getLong(offset + 5) // file_id
        );
        return entity;
    }
    ...
    public void readEntity(Cursor cursor, Track entity, int offset) {
        ...
        entity.setBlock_id(cursor.isNull(offset + 4) ? null : cursor.getLong(offset + 4));
        entity.setFile_id(cursor.isNull(offset + 5) ? null : cursor.getLong(offset + 5));
     }
    ...
    /** Internal query to resolve the "trackList" to-many relationship of Block. */
    public List<Track> _queryBlock_TrackList(Long block_id) {
        synchronized (this) {
            if (block_TrackListQuery == null) {
                QueryBuilder<Track> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.Block_id.eq(null));
                block_TrackListQuery = queryBuilder.build();
            }
        }
        Query<Track> query = block_TrackListQuery.forCurrentThread();
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getBlockDao().getAllColumns());
            builder.append(',');
            SqlUtils.appendColumns(builder, "T1", daoSession.getStorageFileDao().getAllColumns());
            builder.append(" FROM TRACK T");
            builder.append(" LEFT JOIN BLOCK T0 ON T.'BLOCK_ID'=T0.'_id'");
            builder.append(" LEFT JOIN STORAGE_FILE T1 ON T.'FILE_ID'=T1.'_id'");
            builder.append(' ');
    ...
    protected Track loadCurrentDeep(Cursor cursor, boolean lock) {
        ...
        offset += daoSession.getBlockDao().getAllColumns().length;

        StorageFile storageFile = loadCurrentOther(daoSession.getStorageFileDao(), cursor, offset);
        entity.setStorageFile(storageFile);
