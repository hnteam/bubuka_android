...
final public class MessageFormatter {
    ...
    final public static String arrayFormat(final String messagePattern,
        ...
        for (L = 0; L < argArray.length; L++) {
            ...
            if (j == -1) {
                ...
                if (i == 0) { // this is a simple string
                    return lineWithStacktrace(messagePattern, throwableCandidate);
                } else { // add the tail string which contains no variables and return
                    // the result.
                    sbuf.append(messagePattern.substring(i, messagePattern.length()));
                    return lineWithStacktrace(sbuf.toString(),
                            throwableCandidate);
                }
            } else {
        ...
        if (L < argArray.length - 1) {
            return lineWithStacktrace(sbuf.toString(), throwableCandidate);
        } else {
            return sbuf.toString();
        }
    }

    final static String lineWithStacktrace(String line, Throwable exception) {
